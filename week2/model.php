...
function get_serie_table($series, $pdo){
    ...
    foreach($series as $key => $value){
        ...
        <tr>
            ...
            <td>'.(get_username($pdo, $value['user'])['firstname']." ".get_username($pdo, $value['user'])['lastname']).'</td>
            <td><a href="/DDWT18/week2/serie/?serie_id='.$value['id'].'&user_id='.$value['user'].'" role="button" class="btn btn-primary">More info</a></td>
        </tr>
        ';
    }
    $table_exp .= '
 ...
 */
function add_serie($pdo, $serie_info, $userid){
 ...
 * @return array
 */
function update_serie($pdo, $serie_info, $userid, $user){
    if ($userid != $user) {
        return [
            'type' => 'warning',
            'message' => 'You are not allowed to edit this series'
        ];
    }
    /* Check if all fields are set */
    if (
...
function remove_serie($pdo, $serie_id){
    ...
    if ($current_user != $serie_info['user']) {
        return [
            'type' => 'danger',
            'message' => 'You are not allowed to delete this series'
        ];
    }
...
function register_user($pdo, $form_data) {
    if (
        ...
        return [
            'type' => 'danger',
            'message' => 'Please enter an username, a password, your first name and your last name'
        ];
    }
...
created!', (get_username($pdo, $_SESSION['user_id'])['firstname']) . " " . get_username($pdo, $_SESSION['user_id'])['lastname'])
    ];
...
{
    if (empty($form_data['username']) or
        empty($form_data['password'])) {
        return [
            'type' => 'warning',
            'message' => 'Please fill in both your username and password'
        ];
    }
    ...
    /*Return an error is the username does not exist*/
    if (empty($user_info)) {
        return [
            'type' => 'warning',
    ...
    if (!password_verify($form_data['password'], $user_info['password'])) {
        ...
        $feedback = [
            ...
            'message' => sprintf('%s, you were logged in successfully!',
                (get_username($pdo, $_SESSION['user_id'])['firstname']) . " " . get_username($pdo, $_SESSION['user_id'])['lastname'])
        ];
        redirect(sprintf('/DDWT18/week2/myaccount/?error_msg=%s&user_id=%s',
            json_encode($feedback), $_SESSION['user_id']));
    }
}
...
function logout_user($pdo) {
    ...
    $feedback = [
        'type' => 'success',
        'message' => sprintf('%s, you have been succesfully logged out!', (get_username($pdo, $_SESSION['user_id'])['firstname'])." ".get_username($pdo, $_SESSION['user_id'])['lastname'])
    ];
    return $feedback;
}
